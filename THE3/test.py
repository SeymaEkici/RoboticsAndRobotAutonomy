...
class SearchAlgorithms:
    ...
    def enqueue_p(self, cost, node):
        self.priority_queue.append((cost, node))
        self.priority_queue.sort(key=lambda x: x[0])  # Maliyete göre sırala
    ...
    def dijkstra(self):
        self.enqueue_p(0, self.grid_world.start)
        costs = {self.grid_world.start: 0}  
        visited_nodes = set()  

        while self.priority_queue:
            cost, current_node = self.dequeue_p()
            ...
            for neighbor in current_node.neighbors:
                if neighbor in visited_nodes or neighbor.color == 'black':
                    continue
    ...
    for row in grid_world.grid:
        for node in row:
            if node.color == "path":
                path_length += 1
            if node.color == "blue":
                number_of_visited_grid += 1
            #print(f"Node ID: {node.id}, Color: {node.color}")
    
    print(f"number of visited grid = {number_of_visited_grid+path_length}")
    print(f"path length = {path_length}")
    end_time = time.time()
    print(f"time = {float(end_time-start_time)} sec")
